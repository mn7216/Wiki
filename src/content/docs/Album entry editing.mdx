...
Handling albums with multiple editions is documented on [the FAQ page](/docs/pinned/vocadb-editing-faq#if-an-album-was-released-as-multiple-editions-such-as-regularlimited-is-there-need-to-create-separate-entries).
...
If an album has multiple entries due to differences in tracklists, include this information in the title, such as "(Regular Edition)" or "(Kagamine Rin version)," if appropriate. Link these album entries using the album description and/or external links.
...
If the original description is in a non-English language, an English translation can be added/moved in a separate field below.

### Record type

If unsure which type to use, try to find out how the artist(s) themselves categorize the album. For more elaborate descriptions, refer to Wikipedia, Discogs, or Musicbrainz. More details are available on the [Album types and artist participation](/docs/albums/album-types-and-artist-participation) page.
...
Abbreviation of "extended play". Contains 3-4 **original** tracks.
...
Typically a collection of previously published songs, whether published on video sharing sites or gathered from one or more earlier albums. For example, "best of" collections.
...
If the album has no artist credits available, use the [entry missing artists tag](https://vocadb.net/T/2808/entry-missing-artists).

If the vocalist is uncredited, the artist entry [Unknown vocalist(s)](https://vocadb.net/Ar/23554) can be used. The entry [Unknown vocal synth(s)](https://vocadb.net/Ar/128519) or any of its derived artist entries can also be used to further narrow down the type of vocalist.
...
- Artists with support status do not appear in the "artist string" of the album.
- On the album entry page, support artists are shown in parentheses and after all other artists.
- Albums or songs where the artist has support status do not appear on the first tab of the artist's entry page, under recent or popular albums.
- These albums also do not appear on the artist's "main albums" tab.
...
Clicking a track's name opens the "Track properties" window, which can be used to add artists to the song entry (track). Only artists added in the ["Artists" tab](/docs/albums/album-entry-editing#tab-Artists) are visible here. When the album entry is saved, the new tracks are created as song entries.
...
The "custom track" feature (no song entry) is intended for out of scope tracks that do not meet inclusion requirements. [Secret Sky 2020 (DJ Mix)](https://vocadb.net/Al/32323) for example.


## tab: Additional pictures

Note that the main picture is edited on the "Basic Info" tab. Only upload pictures relevant to the album.
